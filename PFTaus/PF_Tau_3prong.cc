...
#pragma HLS UNROLL
	 ...
	 if(n_prongs_found < 3 && n_taus < N_TAUS){
	   if(seed_hadron.et > algo_config.one_prong_seed){
	     ...
	     tau_cands[n_taus].iso_charged = iso_sum_charged_hadron + prong_cands[1].et; 
...
//modified three prong code to account for check of whether a pf_cand is a charged hadron or not
 ...
 #pragma HLS UNROLL
   ...
   {
		 int n_found_prongs = 0;
		 pf_charged_t seedhadron = pf_charged[idx];
      ...
      {
 #pragma HLS UNROLL
			 pf_charged_t temphadron = pf_charged[jdx];
